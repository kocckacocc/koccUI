...
local spacing = 1
local maxbars = 8
local width, height = 160, maxbars*(barheight+spacing)-spacing
local maxfights = 10
local reportstrings = 10
local texture = "Interface\\Addons\\alDamageMeter\\media\\UI-StatusBar"
local backdrop_color = {0,0,0,0.25}
local border_color = {1,1,1,0}
local border_size = 0
...
local UpdateBars = function()
	...
	for i = 1, #barguids do
		...
		bar[i]:SetStatusBarColor(color.r, color.g, color.b, 0.5)
		if sMode == DAMAGE or sMode == SHOW_COMBAT_HEALING then
